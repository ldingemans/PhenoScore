...
class LimeTabularExplainerOvr(LimeTabularExplainer):
    ...
    def __init__(self,
                 ...
                 calculate_stability_indices=False):
        ...
        self.penalty = penalty
        self.base = LimeBaseOvr(
            kernel_fn, verbose=verbose, random_state=self.random_state, penalty=self.penalty)
        self.calculate_stability_indices = calculate_stability_indices
    # Override explain_instance to return also Lime Ridge predictions on the generated points

    def predict_wrapper(self, data, predict_fn, classifier_args):
        return predict_fn(data, classifier_args)

    def Resnik_BMA(self, perturbed_samples, classifier_args):
        from sklearn.preprocessing import MinMaxScaler
        from sklearn.preprocessing import StandardScaler
        import pandas as pd
        print('Resnik BMA!')
        perturbed_samples = pd.DataFrame(perturbed_samples)
        if perturbed_samples.shape[1] == 1:
            perturbed_samples = perturbed_samples.T
        perturbed_samples.columns = classifier_args['mlb_classes']

        hpo_terms_per_sample = []
        for i in range(len(perturbed_samples)):
            hpo_terms_per_sample.append(
                list(perturbed_samples.loc[0, perturbed_samples.loc[i, :] == 1].index))
        Resnik_BMA_similarities = []
        for i in range(len(hpo_terms_per_sample)):
            Resnik_BMA_similarity = classifier_args['simscorer'].calc_similarity(
                hpo_terms_per_sample[i], hpo_terms_per_sample[0])
            Resnik_BMA_similarities.append(Resnik_BMA_similarity)

        Resnik_BMA_similarities = np.array(
            Resnik_BMA_similarities).reshape(-1, 1)
        scaler = StandardScaler()
        normalized_similarities = scaler.fit_transform(Resnik_BMA_similarities)

        # Convert the normalized similarities to distances
        Resnik_BMA_distances = [1/(similarity+0000.1)
                                for similarity in normalized_similarities]
        return np.array(Resnik_BMA_distances).ravel()
    ...
    def explain_instance(self,
        ...
        """
        from sklearn.preprocessing import StandardScaler
        if self.calculate_stability_indices:
        ...
        if not (os.path.exists('yss.npy')) or not (os.path.exists('scaled_data.npy')) or not (os.path.exists('distances.npy')) or not (os.path.exists('data_row.npy')) or stability_indices:
...
#     pool.join()
        ...
        for label in labels:
            (ret_exp.intercept[label],
             ...
             ret_exp.score, ret_exp.local_pred) = self.base.explain_instance_with_data(
                ...
                hpo_terms_and_simscorer=[classifier_args['mlb_classes'], classifier_args['simscorer']])
    ...
    """
        ...
        conf_int = refactor_confints_todict(
            means=beta_ridge, st_devs=stdevs_beta, feat_names=used_features)
...
class Sklearn_Lime(BaseEstimator, RegressorMixin):
    ...
    def _get_lime_exp(self, data_row, predict_fn, classifier_args=None):
    ...
    def predict(self, X, predict_function, classifier_args, y=None):
        ...
        my_exp = self._get_lime_exp(
            data_row=X, predict_fn=predict_function, classifier_args=classifier_args)

        return my_exp

    def score(self, X, predict_function, y=None, sample_weight=None, classifier_args=None):
        ...
        # Check if the number of selected features is larger than the number of features
        elif len(exp.local_exp[1]) > self.num_features:
            Rquadro = 0
