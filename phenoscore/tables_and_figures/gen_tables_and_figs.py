...
def get_top_hpo(list_of_hpo_terms: List, invert_negative_correlations: bool) -> pd.DataFrame:
    ...
    for exp_hpo in list_of_hpo_terms:
        ...
        df_exp_here = pd.concat([df_exp_here.iloc[:, 0].str.split(
            '=', expand=True), df_exp_here.iloc[:, 1]], axis=1)
        df_exp_here.columns = ['hpo', 'positive', 'corr']
        df_exp_total = pd.concat(
            [df_exp_total, df_exp_here]).reset_index(drop=True)
    ...
    if invert_negative_correlations == True:
        df_exp_total.loc[df_exp_total['positive'] == '0', 'corr'] = - \
            df_exp_total.loc[df_exp_total['positive'] == '0', 'corr']
    ...
    if len(list_of_hpo_terms) > 4:
        ...
        df_exp_total = df_exp_total[df_exp_total.groupby(
            "hpo")['hpo'].transform('size') > (threshold - 1)]
    ...
    df_summ_hpo = df_summ_hpo[df_summ_hpo['hpo'].isin(
        np.array(abs(df_summ_hpo['corr']).nlargest(10).index))]
    return df_summ_hpo
...
def get_heatmap_from_multiple(list_of_heatmaps: List, fig: plt.Figure,
    ...
    heatmap = add_corners(heatmap, fill_value=None,
                          w=input_img_size[0], h=input_img_size[1])
    mean_face = add_corners(bg_image, fill_value=min_heatmap,
                            w=input_img_size[0], h=input_img_size[1])

    sm = ax.imshow(heatmap, cmap='seismic_r',  vmin=-
                   max_heatmap, vmax=max_heatmap)
