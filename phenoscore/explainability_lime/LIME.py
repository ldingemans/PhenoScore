...
from OptiLIME.stability_utils import Sklearn_Lime
from OptiLIME.utils import bayesian_optimisation   
...
def calc_hpo_test(filtered_hpo: List, classifier_args: Dict) -> np.ndarray:
    ...
    hpo_features = []
    for i, _ in enumerate(these_hpos):
        avg_pt, avg_cont = [], []
...
def prepare_data_for_lime_explanation(x, index_pt, clf, scale_face, scale_hpo, hpo_terms_pt, hpo_terms_cont, simscorer, facial_feature_extractor, id_to_name):
    ...
    if hpo_terms_pt is not None:
        ...
        x_expanded_test = [int(class_ in x[index_pt, -1]) for class_ in mlb.classes_]
        feature_hpo_names = []
...
def explain_prediction(
    ...
    classifier_args, mlb, x_expanded_train, x_expanded_test, feature_hpo_names = prepare_data_for_lime_explanation(x, index_pt, clf, scale_face, scale_hpo, hpo_terms_pt, hpo_terms_cont, simscorer, facial_feature_extractor, id_to_name)


    # Generate a LIME explanation for the facial image.
    if img_path_index_patient is not None:
        exp_face, local_pred_face = facial_lime_explanation(facial_feature_extractor, img_path_index_patient, n_iter, classifier_args)
    else:
    ...
    if hpo_terms_pt is not None:     
        ...
        if optilime_config.optilime:
            def optilime_loss(kernel_width):
                single_lime = initialize_lime_object(feature_hpo_names, mlb, lime_config, optilime_config, kernel_width)
                single_lime.fit(x_expanded_train.to_numpy())
                score = single_lime.score(np.array(x_expanded_test), predict_hpo, classifier_args=classifier_args)
                return score 
        ...
        for _ in range(1):
            try:
                if optilime_config.optilime:
                    ...
                    obj = initialize_lime_object(feature_hpo_names, mlb, lime_config, optilime_config, float(kw[rsquared.argmax()]))
                    obj.fit(x_expanded_train.to_numpy())
                    exp_hpo = obj.predict(np.array(x_expanded_test), predict_hpo, classifier_args=classifier_args)
                    
                    max_rsquared = exp_hpo.score  
                    kernel_width = float(kw[rsquared.argmax()])
                    sum_coeffs = sum(coeff for sublist in exp_hpo.local_exp.values() for _, coeff in sublist)
                    ...
                    obj = initialize_lime_object(feature_hpo_names, mlb, lime_config, optilime_config, kernel_width)
                    obj.fit(x_expanded_train.to_numpy())
                    exp_hpo = obj.predict(np.array(x_expanded_test), predict_hpo, classifier_args=classifier_args)
                    sum_coeffs = sum(coeff for sublist in exp_hpo.local_exp.values() for _, coeff in sublist)
                    max_rsquared = exp_hpo.score
