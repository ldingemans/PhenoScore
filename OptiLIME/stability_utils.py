...
class LimeBaseOvr(LimeBase):
    ...
    def explain_instance_with_data(self,
        ...
        sorted_coefficients = sorted(zip(used_features, easy_model.coef_), key=lambda x: np.abs(x[1]), reverse=True)
...
class LimeTabularExplainerOvr(LimeTabularExplainer):
    ...
    def Resnik_BMA(self, perturbed_samples, classifier_args):
        ...
        for i in range(len(perturbed_samples)):
            hpo_terms_per_sample.append(list(perturbed_samples.loc[0,perturbed_samples.loc[i,:] == 1].index))
        Resnik_BMA_similarities = []
        for i in range(len(hpo_terms_per_sample)):
            Resnik_BMA_similarity = classifier_args['simscorer'].calc_similarity(hpo_terms_per_sample[i], hpo_terms_per_sample[0])
            Resnik_BMA_similarities.append(Resnik_BMA_similarity)

        Resnik_BMA_similarities = np.array(Resnik_BMA_similarities).reshape(-1, 1)
        scaler = StandardScaler()
        normalized_similarities = scaler.fit_transform(Resnik_BMA_similarities)

        # Convert the normalized similarities to distances
        Resnik_BMA_distances = [1/(similarity+0000.1) for similarity in normalized_similarities]
...
class Sklearn_Lime(BaseEstimator, RegressorMixin):
    ...
    def predict(self, X, predict_function,classifier_args, y=None):
        ...
        my_exp = self._get_lime_exp(data_row=X, predict_fn=predict_function,classifier_args=classifier_args)

        return my_exp

    def score(self, X, predict_function, y=None, sample_weight=None, classifier_args=None):
        
        exp = self.predict(X,predict_function,classifier_args=classifier_args)
        ...
        elif len(exp.local_exp[1]) > self.num_features: # Check if the number of selected features is larger than the number of features
            Rquadro = 0
